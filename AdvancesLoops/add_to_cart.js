...
for (let i = 0; i < products.length; i++) {
    ...
    <div>
    <button class="bg-blue-500 text-white px-4 py-2 rounded add_to_cart">Add to Cart</button>
...
addToCartBtns.forEach((btn, index) => {
    btn.addEventListener("click", () => {
        cart.push(products[index]); // add the correct product
        console.log("Cart:", cart);
        msg.textContent = `${products[index].name} added to cart `;
    });
});
...
discount.addEventListener('click', () => {
    let dis = cart.map(product => product.price * 0.9)
